...
c or adapted to ASTRA needs
	write(*,*)"The routine GLF161 is obsolete. Use GLF161A"
	stop
      epsilon  = 1.e-10
...
c
      do j=0,jmaxm
	 work(j+1,1) = chii_m(j) ! \chi_i, m^2/s
	 work(j+1,2) = chie_m(j) ! \chi_e, m^2/s
	 work(j+1,3) = diff_m(j) ! D, electron diffusivity, m^2/s
	 work(j+1,4) = etaphi_m(j) !
	 work(j+1,5) = etapar_m(j) !
	 work(j+1,6) = etaper_m(j) !
	 work(j+1,7) = exch_m(j)   !
	 work(j+1,8) = egamma_m(j)  !
	 work(j+1,9) = gamma_p_m(j) !
	 work(j+1,10) = anrate_m(j)  !
	 work(j+1,11) = anrate2_m(j) !
	 work(j+1,12) = anfreq_m(j)  !
	 work(j+1,13) = anfreq2_m(j) !
      enddo
      if (jna .lt. NA1)	then
	 do j=jna+1,NA1
	    work(j,1) = 0.
