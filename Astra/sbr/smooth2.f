...
C----------------------------------------------------------------------|
	subroutine	SMOOTH2(ALFA,FO,FN)
	implicit none
	double precision	ALFA,FO(*),FN(*)
	include	'for/parameter.inc'
	include	'for/const.inc'
	include 'for/status.inc'
	call	SGLAZH2(ALFA,NA1,FO,RHO,NA1,FN,RHO)
	end
C======================================================================|
	subroutine	SGLAZH2(ALFA,NO,FO,XO,N,FN,XN) ! same as SMOOTH
C----------------------------------------------------------------------|
	implicit none
	...
	do	1	j=2,N
	P(j)	=ALFA*(XO(j)/XO(NO))/(XN(j)-XN(j-1))/XO(NO)**2
C	P(j)	=ALFA*sqrt(XO(j)/XO(NO))/(XN(j)-XN(j-1))/XO(NO)**2
C	P(j)	=ALFA/(XN(j)-XN(j-1))/XO(NO)**2
 1	continue
	P(1)	=0.
	FN(1)	=FO(1)
	I	=1
