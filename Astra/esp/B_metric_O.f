...
!         include 'dim.inc'
!         include 'compol.inc'
!         common/com_but/ cbut_b(nrp)
!         dimension F_but(nrp),C_but(nrp)
!         dimension alp22(nrp),alp33(nrp),delsc(nrp),delv(nrp)
!         dimension alp33k(nrp),delvk(nrp),delsk(nrp),cur_I(nrp)
!
!         sqrt(x)=dsqrt(x)
!
!
...
!             i=iplas
!           f_but(i)=0.d0
!           dpsi=0.5d0*(psia(i)-psia(i-1))*(psim-psip)
!           ffp=ffprim_b(psia(i))
!           sqroot=sqrt( (f(i)**2-f(i+1)**2 + f)
!           f_but(iplas-1)=
!
!          do i=iplas-2,1,-1
!           dpsi=0.5d0*(psia(i+2)-psia(i))*(psim-psip)
!           ffp=ffprim_b(psia(i+1))
!           f_but(i)=sqrt(f_but(i+1)**2-2.d0*ffp*dpsi)
!          enddo
!
!          skcen=0.d0
!         do j=2,nt1
...
!
!            r0=r(1,2)
!            curj_b=r0*pprim_b(1.d0)+ffprim_b(1.d0)/r0
!            C_but(1)=curj_b*skcen
!            cbut_b(1)=f(1)*curj_b/rm
!
!        do i=2,Iplas
!            summj=0.d0
!        do j=2,Nt1
...
!          psn=(3.d0*psia(i)+psia(i-1))*0.25d0
!         endif
!            curj_b=r0*pprim_b(psn)+ffprim_b(psn)/r0
!            summj=summj+curj_b*sqk
!        enddo
!            C_but(i)=summj
!        enddo
!    
!         call metcof(alp22,alp33,delsc,delv,ac0,skcen,cur_I,
!     *                     alp33k,delsk,delvk)
!
!        do i=2,Iplas-1
!
!        cbut_b(i)=0.5d0*((f(i-1)+f(i))*(C_but(i)-C_but(i-1))
!     *         - (cur_I(i-1)+cur_I(i))*(F_but(i)-F_but(i-1)))/delvk(i)
!
!        enddo
!        
!        cbut_b(iplas)=0.5d0*(f(iplas-1)*(C_but(iplas)-C_but(iplas-1))
!     *         - 2.d0*cur_I(iplas-1)*(-F_but(iplas-1)))/delvk(iplas)
!
!
!        return
!        end
