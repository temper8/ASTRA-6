...
!	 call out42(n_pr,a_print,num,apr)
 ...
 234              continue                             !
         ...
         if(itl.ge.Nitl) then
            !write(6,*) 'limit number of ext. iterations is exeeded'
            !write(6,*) 'itl = ',itl,' Nitl = ',Nitl
            go to 1111
         endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
C	       write(6,*) 'betplx/betpol',zcoin
       ...
       if(erru.le.eps0 .AND. crz.lt.epscrz .AND. itl.ge.0
     *   .AND. ich.eq.0) go to 1111

       omega = omg
       sigma = sigm
!	if(erru.le.epsin.OR. itin.ge.Nitin)then
!
!       n_pr       = 5
!	 a_print(1) = iter
!	 a_print(2) = erru
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	   ...
	   alf0n=alf0
         !write(6,*) '..................................................'
         !write(6,*) 'Convergence of free bound. equilibrium iterations:'
         !write(6,*) '  iter = ',iter,' nrun = ',nrun
         !write(6,*) '  itl  = ',itl ,' Nitl = ',Nitl
         !write(6,*) '  erru = ',erru
         !write(6,*) '  crz  = ',crz
...
!          if(kstep.gt.0) then
!           call wrd
!           pause 'eq_ax:wrd before call right0 ///'
!          endif
