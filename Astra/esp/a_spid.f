	subroutine a_spid
c(itrel,relerr) version 1-FEB-09 for Astra 6.2.xx
...
c		relerr - relerr for convergency
     ...
     *		yrtor,ybtor,yroc1,yroc,yrocnew,yipl,yyipl
	 common /com_flux/jneql,jnteta,yrout,yzout
	 real*8
...
!	 Read(1,*) keyctr	!= 0 
	 ...
	 Read(1,*) key_ini	!(1/0) start with astra/EQDSK-SPIDER profiles	
...
c=========================================================================
	if(nstep.eq.0) then
	   ...
	   if(key_dmf.eq.-2) write(*,*)
     .			'external voltage UEXT(t) for current diffusion'
	endif
	call  kpr_calc(kpr)
...
!=======================================BPEMEHHO read ^^^^^^^^^^^^ 
	if(yneold.ne.0.d0) then
	   if(nequil.ne.yneold.and.key_dmf.ne.0) then
	      nequil = yneold
	      write(*,*) 'exp/equ/spiderin.dat: for current diffusion ',
     .			'(key_dmf=-2,-3) NEQUIL can not be changed' 
	   endif
	endif
	if(nequil .ne. yneold) then		! 1st call
!aai		icount = 3
C...Initial iterrations start every time when NEQUIL > = 43 was changed
C	   write(*,*) 'Attention! 	Metric for SPIDER solver:'
C	   write(*,*) 'boudary points Rij,Zij',
C     . 		' must be in contr clock wise direction'
C	   write(*,*)
C     .		'new mesh point numbers for SPIDER equilibrium solver'
	   jnj_p0	=NEQUIL
	   jni_p0	=(100*(NEQUIL-jnj_p0))
	   if(jni_p0.le.43) then
	      jni_p0 = 44
	      write(*,*) 'for spider equilibrium is changed:'
	      write(*,*)'OLD NEQUIL = ', NEQUIL,'NEW NEQUIL =',
     .			jnj_p0 +0.44d0
	      NEQUIL = jnj_p0 + 0.44d0
	   endif
	   yneold	=NEQUIL
C	   write(*,*) 'N poloidal = Int(NEQUIL) 	= ', jnj_p0
C	   write(*,*) 'N radial = 100*(NEQUIL - Int(NEQUIL)) = ',jni_p0
	else
	   jnstep	=1
	endif
	jnteta=jnj_p0
	...
	if (NBND .gt. NBNDX) then
	   write(*,'(2(A,I3))')' >>> SPIDER >>> # of boundary points (',
     >			NBND,')  cannot exceed ',NBNDX
	   return
	elseif (NBND .eq. 0) then	! No boundary points are given
C	   write(*,'(A)')' >>> SPIDER >>> 3M boundary will be used'
	endif
	call	BNDRY1(rzbnd)
	if (kpr .eq. 2)	then
!	   write 'dat/spidat.dat' for SPIDER stand alone analysis
...
c======================================================================
C	write(*,*) NBND,NBNT,NEQUIL,jnteta,jneql
c============================================= start of spider
        call aspid_flag(1)
...
C	write(*,*)jneql,jnteta
C	write(*,'(10F7.3)')((yrout(ji,jj),jj=1,jnteta),ji=1,1)
C	write(*,'(10F7.3)')((yzout(ji,jj),jj=1,jnteta),ji=1,1)
	do ji=jneql,2,-1
	   yrmax=-99999.d0
	   yrmin=99999.d0
	   yzmax=-99999.d0
...
C Redraw magnetic surfaces:
	implicit	none
	integer		j,jj,jt,jifnew,YS0,DYLET
	integer		nrp,ntp,jneql,jnteta
	parameter (nrp=256, ntp=256)	! As defined in  'dimpl1.inc'
	integer		PT(2*nrp*ntp)
	double precision YSC8,yrout(nrp,ntp),yzout(nrp,ntp)
	common /com_flux/jneql,jnteta,yrout,yzout
	save	PT
C----------------------------------------------------------------------|
	jt = 0
	do  65	J=2,jneql,1+jneql/8
	   if (jifnew .eq. 0)	then		! Erase
	      jj = 31	! Draw the old flux surfaces with shadow color
	      jj = 0	! Erase the old flux surfaces
	      call colovm(jj)
	      jj = 0
	      call d2polyline(jj,PT(jt+1),jnteta)
	   endif
	   if (j .le. jneql)	call colovm(14)
	   do	JJ=1,jnteta
	      PT(jt+2*JJ-1) = 10.*YSC8*yrout(j,jj)
	      PT(jt+2*JJ)   = 10.*(YS0-YSC8*yzout(j,jj))
	   enddo
	   jj = 0
	   call d2polyline(jj,PT(jt+1),jnteta)
	   jt = jt+2*jnteta
